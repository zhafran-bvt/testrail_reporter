    ...
    **Validates: Requirements 2.1**
        ...
        **Validates: Requirements 2.1**
        ...
        for i, test in enumerate(result["tests"]):
            ...
            if test["status_id"] in [1, 2, 3, 4, 5]:
                assert test["status_name"] in valid_statuses, \
    ...
    def test_invalid_run_id_returns_400(self):
        ...
        assert resp.status_code == 400, \
            f"Expected 400 for run_id=0, got {resp.status_code}"
    ...
    **Validates: Requirements 3.8, 3.9**
    ...
    def test_invalid_file_type_rejected(self, case_id, invalid_content_type):
        ...
        **Validates: Requirements 3.8**
        ...
        files = {
            "file": ("test_file.bin", BytesIO(file_content), invalid_content_type)
        }
        ...
        assert resp.status_code == 400, \
            f"Expected 400 for invalid type {invalid_content_type}, got {resp.status_code}"
        ...
        assert "type" in detail_lower or "allowed" in detail_lower, \
    ...
    def test_valid_file_type_accepted(self, case_id, valid_content_type):
        ...
        **Validates: Requirements 3.8**
        ...
        files = {
            "file": (f"test_file{ext}", BytesIO(file_content), valid_content_type)
        }
        ...
        if resp.status_code == 400:
            ...
            assert "type" not in detail or "allowed" not in detail, \
    ...
    def test_oversized_file_rejected(self, case_id, file_size_mb):
        ...
        **Validates: Requirements 3.9**
        ...
        files = {
            "file": ("large_file.png", BytesIO(file_content), "image/png")
        }
        ...
        # Should be rejected with 400
        assert resp.status_code == 400, \
            f"Expected 400 for oversized file ({file_size_mb}MB)"
        
        # Error message should mention size
        result = resp.json()
        assert "detail" in result, "Response should contain 'detail'"
        detail_lower = result["detail"].lower()
        assert "size" in detail_lower or "25" in result["detail"] or "mb" in detail_lower, \
    ...
    def test_valid_file_size_accepted(self, case_id, file_size_kb):
        ...
        **Validates: Requirements 3.9**
        ...
        files = {
            "file": ("valid_file.png", BytesIO(file_content), "image/png")
        }
        ...
        if resp.status_code == 400:
            result = resp.json()
            detail = result.get("detail", "").lower()
            assert "size" not in detail and "25" not in detail and "mb" not in detail, \
...
class TestRunEditModalProperties(unittest.TestCase):
    ...
    **Validates: Requirements 1.1, 1.2, 1.3**
    ...
    def test_run_update_api_accepts_valid_payload(
        self, run_id, run_name, description, refs
    ):
        """
        **Feature: test-run-case-management, Property 2: Run Update API Integration**
    ...
    def test_run_update_dry_run_returns_preview(
        self, run_id, run_name, description, refs
    ):
        """
        **Feature: test-run-case-management, Property 3: Cancel Closes Modal**
        **Validates: Requirements 1.3**
        ...
        assert result["payload"].get("name") == run_name.strip(), \
            "payload.name should match"
...
class TestCaseEditModalProperties(unittest.TestCase):
    ...
    **Validates: Requirements 3.1, 4.1, 4.2, 3.7**
    ...
    def test_case_update_api_accepts_valid_payload(
        self, case_id, title, refs, bdd_scenarios
    ):
        """
        ...
        def update_case(cid, body):
            if cid == case_id:
                return {
                    ...
                    "refs": body.get("refs", refs),
                    "custom_bdd_scenario": body.get(
                        "custom_bdd_scenario", bdd_scenarios
                    ),
                }
    ...
    def test_attachment_upload_success(
        self, case_id, valid_content_type, file_size_kb, attachment_id
    ):
        """
        **Feature: test-run-case-management, Property 18: Attachment Upload Success**
        **Validates: Requirements 3.7**
        ...
        fake = types.SimpleNamespace()
        fake.add_attachment_to_case = lambda cid, path, fname: {
        ...
        assert result["attachment"]["id"] == attachment_id, \
            f"attachment.id should be {attachment_id}"
        assert result["attachment"]["content_type"] == valid_content_type, \
    ...
    **Validates: Requirements 2.3, 6.1, 7.2, 7.3**
    ...
    def test_status_badge_color_mapping(self, status_id):
        ...
        **Validates: Requirements 6.1**
        ...
        assert expected_class is not None, \
            f"Status ID {status_id} should have a badge class"
        
        # Verify the class name follows the expected pattern
        assert expected_class.startswith("badge-"), \
            f"Badge class should start with 'badge-': {expected_class}"
    ...
    def test_run_name_header_display(self, run_id, run_data):
        ...
        **Validates: Requirements 7.3**
        ...
        assert "run_name" in result, \
            "Response should contain 'run_name' for header display"
        assert result["run_name"] == run_data["name"], \
            f"run_name should match: expected '{run_data['name']}'"
    ...
    def test_back_navigation_returns_to_runs(self):
        ...
        **Validates: Requirements 7.2**
        ...
        fake = types.SimpleNamespace()
        run_data = {
            "id": 1, "name": "Test Run", "description": None, "is_completed": False
        }
        ...
        assert "run_id" in result, \
            "Response should contain 'run_id' for navigation context"
    ...
    **Validates: Requirements 6.1, 6.2, 6.3, 6.4, 6.5**
    ...
    def test_api_returns_correct_status_name(self, run_id, run_data, status_id):
        ...
        **Validates: Requirements 6.1, 6.2, 6.3, 6.4, 6.5**
        ...
        assert test["status_name"] == expected_name, \
