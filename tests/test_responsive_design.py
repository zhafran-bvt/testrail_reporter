...
def test_create_forms_grid_responsive():
    ...
    # Desktop: 3 columns
    assert re.search(r'\.create-forms-grid\s*\{[^}]*grid-template-columns:\s*repeat\(3,\s*1fr\)', html_content), \
        "Desktop create forms grid (3 columns) not found"
    
    # Tablet: 2 columns
    tablet_section = re.search(
        r'@media\s*\(max-width:\s*1024px\)\s*and\s*\(min-width:\s*768px\)\s*\{(.*?)\}',
        html_content,
        re.DOTALL
    )
    assert tablet_section, "Tablet media query section not found"
    assert re.search(r'\.create-forms-grid\s*\{[^}]*grid-template-columns:\s*repeat\(2,\s*1fr\)', tablet_section.group(1)), \
        "Tablet create forms grid (2 columns) not found"
    
    # Mobile: 1 column
    mobile_section = re.search(
        r'@media\s*\(max-width:\s*767px\)\s*\{(.*?)(?=@media|\Z)',
        html_content,
        re.DOTALL
    )
    assert mobile_section, "Mobile media query section not found"
    assert re.search(r'\.create-forms-grid\s*\{[^}]*grid-template-columns:\s*1fr', mobile_section.group(1)), \
...
def test_touch_friendly_buttons():
    ...
    touch_query = re.search(
        r'@media\s*\(max-width:\s*767px\)\s*and\s*\(hover:\s*none\)\s*\{(.*?)\}',
        html_content,
        re.DOTALL
    )
    ...
    assert re.search(r'min-height:\s*44px', touch_query.group(1)), \
        "Touch-friendly min-height (44px) not found"
...
def test_manage_subsection_responsive():
    ...
    assert re.search(r'\.manage-subsection\s*\{[^}]*padding:\s*16px', mobile_section.group(1)), \
...
def test_entity_card_actions_responsive():
    ...
    assert re.search(r'\.entity-card-actions\s*\{[^}]*flex-direction:\s*column', mobile_section.group(1)), \
        "Mobile entity card actions flex-direction not found"
    
    # Check for full width buttons
    assert re.search(r'\.btn-edit,\s*\.btn-delete\s*\{[^}]*width:\s*100%', mobile_section.group(1)), \
...
def test_subsection_controls_responsive():
    ...
    assert re.search(r'\.subsection-controls\s*\{[^}]*flex-direction:\s*column', mobile_section.group(1)), \
        "Mobile subsection controls flex-direction not found"
    
    # Check for full width search inputs
    assert re.search(r'\.subsection-controls\s+input\[type="search"\]\s*\{[^}]*width:\s*100%', mobile_section.group(1)), \
...
def test_landscape_orientation_support():
    ...
    landscape_query = re.search(
        r'@media\s*\(max-width:\s*767px\)\s*and\s*\(orientation:\s*landscape\)',
        html_content
    )
