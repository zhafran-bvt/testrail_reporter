...
class TestReportGenerator(unittest.TestCase):
    ...
    @patch('testrail_daily_report.download_attachment')
    @patch('testrail_daily_report.render_html')
    def test_generate_report_video_attachment_paths(self, mock_render_html, mock_download_attachment, mock_transcode_video):
        fake_client = MagicMock()
        fake_client.base_url = "http://fake-testrail.com"
        fake_client.timeout = 5.0
        fake_client.max_attempts = 2
        fake_client.backoff = 1.0
        fake_client.get_project.return_value = {"name": "Project"}
        fake_client.get_plan.return_value = {"name": "Video Plan", "entries": [{"runs": [{"id": 301, "name": "Video Run"}]}]}
    ...
    def test_generate_report_multiple_runs_snapshot(self, mock_render_html, mock_download_attachment):
        ...
        fake_client.backoff = 1.0
        fake_client.get_project.return_value = {"name": "Project"}
        fake_client.get_plan.return_value = {"name": "Plan", "entries": [{"runs": [{"id": 10, "name": "Alpha"}, {"id": 20, "name": "Beta"}]}]}
    ...
    def test_generate_report_mixed_attachment_runs(self, mock_render_html, mock_download_attachment):
        ...
        fake_client.backoff = 1.0
        fake_client.get_project.return_value = {"name": "Project"}
        fake_client.get_plan.return_value = {"name": "Plan", "entries": [{"runs": [{"id": 30, "name": "NoFiles"}, {"id": 40, "name": "WithFiles"}]}]}
    ...
    def test_generate_report_html_contains_runs(self, mock_download_attachment):
        ...
        fake_client.backoff = 1.0
        fake_client.get_project.return_value = {"name": "Project"}
        fake_client.get_plan.return_value = {"name": "Plan", "entries": [{"runs": [{"id": 50, "name": "First"}, {"id": 60, "name": "Second"}]}]}
    ...
    def test_video_transcode_disabled(self, mock_render_html, mock_download_attachment, mock_transcode_video):
        ...
        fake_client.backoff = 1.0
        fake_client.get_project.return_value = {"name": "Project"}
        fake_client.get_plan.return_value = {"name": "Video Plan", "entries": [{"runs": [{"id": 401, "name": "Video"}]}]}
        ...
        fake_client.get_attachments_for_test.return_value = [{"id": 700, "name": "clip.mov", "result_id": 900, "size": 1000}]
    ...
    def test_keep_attachment_files_flag(self, mock_render_html, mock_download_attachment):
        ...
        fake_client.backoff = 1.0
        fake_client.get_project.return_value = {"name": "Project"}
        fake_client.get_plan.return_value = {"name": "Plan", "entries": [{"runs": [{"id": 501, "name": "Inline"}]}]}
        fake_client.get_plan_runs.return_value = [501]
        fake_client.get_tests_for_run.return_value = [{"id": 42, "title": "Attachment", "status_id": 1, "priority_id": 1, "assignedto_id": 1}]
        fake_client.get_results_for_run.return_value = [{"id": 42, "test_id": 42, "status_id": 1}]
        fake_client.get_attachments_for_test.return_value = [{"id": 600, "name": "pic.png", "result_id": 42, "size": 512}]
    ...
    def test_generate_report_snapshot_disabled(self, mock_render_html, mock_download_attachment):
        ...
        fake_client.backoff = 1.0
        fake_client.get_project.return_value = {"name": "Project"}
        fake_client.get_plan.return_value = {"name": "Plan", "entries": [{"runs": [{"id": 400, "name": "Single"}]}]}
    ...
    def test_attachment_batching_respects_env(self, mock_render_html, mock_compress_image, mock_download_attachment):
        ...
        fake_client.backoff = 1.0
        fake_client.get_project.return_value = {"name": "Project"}
        fake_client.get_plan.return_value = {"name": "Plan", "entries": [{"runs": [{"id": 700, "name": "Batch"}]}]}
