...
from app.models.requests import AddTestResult, ManagePlan, ManageRun, ReportRequest
...
class TestValidationErrorDetail:
    ...
    def test_manage_run_validation_provides_constraint_errors(self, include_all, case_ids):
        ...
        try:
            ManageRun(name="Test Run", include_all=include_all, case_ids=case_ids)
        except ValidationError as e:
            errors = e.errors()
            ...
            constraint_errors = [
                err for err in errors if "case_ids" in err["msg"].lower() or "include_all" in err["msg"].lower()
            ]
