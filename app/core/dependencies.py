...
def get_plans_cache() -> TTLCache:
    ...
    return TTLCache(ttl_seconds=config.PLANS_CACHE_TTL, maxsize=config.PLANS_CACHE_MAXSIZE)


@lru_cache()
def get_runs_cache() -> TTLCache:
    """Get runs cache instance."""
    return TTLCache(ttl_seconds=config.RUNS_CACHE_TTL, maxsize=config.RUNS_CACHE_MAXSIZE)


@lru_cache()
def get_dashboard_plans_cache() -> TTLCache:
    """Get dashboard plans cache instance."""
    return TTLCache(ttl_seconds=config.DASHBOARD_PLANS_CACHE_TTL, maxsize=128)


@lru_cache()
def get_dashboard_plan_detail_cache() -> TTLCache:
    """Get dashboard plan detail cache instance."""
    return TTLCache(ttl_seconds=config.DASHBOARD_PLAN_DETAIL_CACHE_TTL, maxsize=64)


@lru_cache()
def get_dashboard_stats_cache() -> TTLCache:
    """Get dashboard stats cache instance."""
    return TTLCache(ttl_seconds=config.DASHBOARD_STATS_CACHE_TTL, maxsize=128)


@lru_cache()
def get_dashboard_run_stats_cache() -> TTLCache:
    """Get dashboard run stats cache instance."""
    return TTLCache(ttl_seconds=config.DASHBOARD_RUN_STATS_CACHE_TTL, maxsize=256)
