...
def test_manage_plan_creates_with_fake_client(monkeypatch):
    ...
    resp = api.post("/api/manage/plan", json={"project": 2, "name": "My Plan", "description": "desc"})
...
def test_manage_run_targets_plan_entry(monkeypatch):
    ...
    resp = api.post("/api/manage/run", json={"project": 1, "plan_id": 99, "name": "Run", "include_all": True})
    assert resp.status_code == 200
...
def test_manage_case_calls_client(monkeypatch):
    ...
    resp = api.post("/api/manage/case", json={"project": 1, "title": "Case title", "bdd_scenarios": "Given\nWhen\nThen"})
    assert resp.status_code == 200
