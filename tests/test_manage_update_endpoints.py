...
def gen_case_update_data(draw):
    ...
    has_valid_field = (
        (result["title"] is not None) or
        (result["refs"] is not None) or
        (result["bdd_scenarios"] is not None and result["bdd_scenarios"].strip() != "")
    )
        ...
        **Validates: Requirements 11.2**
        ...
        for field, value in update_data.items():
            if value is not None:
                ...
                assert updated_plan[field] == value, \
        ...
        if update_data.get("name") is None:
            assert updated_plan["name"] == original_plan["name"], \
                "Unchanged name should preserve original value"
        
        if update_data.get("description") is None:
            assert updated_plan["description"] == original_plan["description"], \
                "Unchanged description should preserve original value"
        
        if update_data.get("milestone_id") is None:
            assert updated_plan["milestone_id"] == original_plan["milestone_id"], \
    ...
    def test_plan_update_rejects_empty_name(self, plan_id):
        ...
        **Validates: Requirements 11.5**
        ...
        for empty_name in empty_names:
            resp = client.put(
                f"/api/manage/plan/{plan_id}",
                json={"name": empty_name, "dry_run": False}
            )
            
            # Should return validation error (422 for Pydantic validation)
            assert resp.status_code == 422, \
                f"Empty name '{repr(empty_name)}' should return 422, got {resp.status_code}"
        ...
        **Validates: Requirements 13.2**
        ...
        for field, value in update_data.items():
            if value is not None:
                ...
                assert updated_run[field] == value, \
                    f"Field '{field}' should have value '{value}', got '{updated_run[field]}'"
        
        # Property: Fields not in update_data should preserve original values
        if update_data.get("name") is None:
            assert updated_run["name"] == original_run["name"], \
                "Unchanged name should preserve original value"
        
        if update_data.get("description") is None:
            assert updated_run["description"] == original_run["description"], \
                "Unchanged description should preserve original value"
        
        if update_data.get("refs") is None:
            assert updated_run["refs"] == original_run["refs"], \
                "Unchanged refs should preserve original value"
            ...
            if value is not None:
                if field == "bdd_scenarios":
                    ...
                    if value.strip():  # Only if non-empty
                        assert "custom_testrail_bdd_scenario" in updated_case, \
                            "BDD scenarios should be in updated case"
                else:
                    assert field in updated_case, f"Field '{field}' should be in updated case"
                    assert updated_case[field] == value, \
                        f"Field '{field}' should have value '{value}', got '{updated_case[field]}'"
        
        # Property: Fields not in update_data should preserve original values
        if update_data.get("title") is None:
            assert updated_case["title"] == original_case["title"], \
                "Unchanged title should preserve original value"
        
        if update_data.get("refs") is None:
            assert updated_case["refs"] == original_case["refs"], \
                "Unchanged refs should preserve original value"
    ...
    def test_case_update_rejects_empty_title(self, case_id):
        ...
        **Validates: Requirements 15.5**
        ...
        for empty_title in empty_titles:
            resp = client.put(
                f"/api/manage/case/{case_id}",
                json={"title": empty_title, "dry_run": False}
            )
...
class TestUpdateEndpointsUnit(unittest.TestCase):
    ...
    def test_update_plan_with_valid_data_returns_updated_entity(self):
        ...
        resp = client.put(
            "/api/manage/plan/123",
            json={"name": "Updated Plan", "description": "New description"}
        )
    ...
    def test_update_plan_dry_run_returns_preview(self):
        ...
        resp = client.put(
            "/api/manage/plan/123",
            json={"name": "Test Plan", "dry_run": True}
        )
    ...
    def test_update_run_with_valid_data_returns_updated_entity(self):
        ...
        resp = client.put(
            "/api/manage/run/456",
            json={"name": "Updated Run", "refs": "JIRA-123"}
        )
    ...
    def test_update_case_with_valid_data_returns_updated_entity(self):
        ...
        resp = client.put(
            "/api/manage/case/789",
            json={"title": "Updated Case", "refs": "TEST-456"}
        )
